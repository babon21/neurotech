...
func NewSessionStorer(sessionName string, keypairs ...[]byte) SessionStorer {
	store, err := redis.NewRediStore(10, "tcp", ":6379", "", []byte("secret-key"))
	...
	store.DefaultMaxAge = int((time.Hour * 12) / time.Second)
	store.Options.HttpOnly = true

	// secure cookie send to server when HTTPS connection
